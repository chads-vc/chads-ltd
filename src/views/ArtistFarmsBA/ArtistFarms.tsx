...
const Farm: React.FC = () => {
  ...
  const [onPresentCoppedModal8] = useModal(<CoppedModal cardId={8} />)
   ...
   const [onPresentCoppedModal15] = useModal(<CoppedModal cardId={15} />)
   const [onPresentCoppedModal16] = useModal(<CoppedModal cardId={16} />)

   const [onPresentCoppedModal17] = useModal(<CoppedModal cardId={17} />)

  const earnings = useEarnings(contract)
  
  const rows = chunk(chadletsCards.slice(0,8), 3);
  const cols = chunk(chadletsCards.slice(14,16), 3);
  
  const colsVideo = chunk(chadletsCards.slice(16,17), 3);
  ...
  return (
        ...
        {cols &&cols.map((cardRow, i) => (
          (<StyleVertRow key={i}>
            <StyledRowHeader>
                  ...
                  <StyledVertHeading>
                  Blunt Action Stills<br/>2500 CHADLETs
                  </StyledVertHeading>
                      ...
                      <Button
                        onClick={() => {
                          if (totalCopped[card.index-1] >= card.max_supply) {
                           ...
                           }else if (card.index == 15) {
                             onRedeem("15").then(txnHash => onPresentCoppedModal15());
                           }else if (card.index == 16) {
                             onRedeem("16").then(txnHash => onPresentCoppedModal16());
                           }
                        }}
                        text=""
    ...
    <StyledCards2>
         {/* vertical layout */}
        {colsVideo &&colsVideo.map((cardRow, i) => (
          (<StyleVertRow key={i}>
            <StyledRowHeader>
               	<React.Fragment>
                  <StyledGreekImage src={greekArtist} />
                  <StyledVertHeading>
                  Blunt Action x Whispa<br/>4200 CHADLETs
                  </StyledVertHeading>
                  <StyledWavyCommon src={wavyClipArt} />
                </React.Fragment>
            </StyledRowHeader>
            <StyledVertCardsRow>
              {cardRow.map((card, j) => {
                return (<StyledVertCard key={j}>
                  <StyledCardContent>
                    <StyledVertContent>
                      <video controls style={{'width': '100%'}} src={`https://api.chads.vc/video/${card.index}.mp4`} />
                        ...
                        onClick={() => {
                          if (totalCopped[card.index-1] >= card.max_supply) {
                            ...
                            onRedeem("12").then(txnHash => onPresentCoppedModal12());
                             ...
                             onRedeem("15").then(txnHash => onPresentCoppedModal15());
                           }else if (card.index == 17) {
                             onRedeem("17").then(txnHash => onPresentCoppedModal17());
                           }
                        }}
                        text=""
                        disabled={(earnings.toNumber() < card.pool.points) && (totalCopped[card.index-1] < card.max_supply)} 
